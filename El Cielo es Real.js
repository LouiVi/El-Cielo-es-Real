...
function OnStart()
{    
//app.CopyFile( "Misc/El Cielo es Real.txt", "rEal.txt" );
...
//	txtIcon.SetTextColor( "#4285F4" );
	...
	logo = app.CreateImage( "Img/El Cielo es Real.png",0.65,-1 );
	logo.SetMargins( 0.01, 0.03, 0.01, 0 )
	layScroll.AddChild( logo );
	sI = setInterval(()=>{logo.Animate( "Bounce" );}, 3500);
	//Create text with an external link.
    txt = app.ReadFile( "/storage/emulated/0/Download/a/a.txt", "UTF-8").split("\n\r").join("\r\n");
    
    //alert(txt);app.GetPermission(  )
...
function CreateActionBar()
{
    ...
    //Add title text.
    txtBarTitle = app.AddText( layBar, "El Cielo es Real", 1 )
...
function CreateDrawer()
{
	...
	var img = app.AddImage( layDrawerTop, "Img/El Cielo es Real.png", 0.15 )
	img.SetPosition( drawerWidth*0.06, 0.04 )
	
	//Add user name to top layout.
	var txtUser = app.AddText( layDrawerTop, "El Cielo es Real",-1,-1,"Bold")
    ...
    var listItems = "El Cielo es Real::[fa-book]";
    ...
    lstMenu1.SetItemByIndex( 0, "El Cielo es Real", 21 )
...
function ReadText()
{
	...
	app.TextToSpeech( curLine, 1, 1, ()=>{layScroll.Animate("Tada");ReadText();}, "music", "es-pr" );
